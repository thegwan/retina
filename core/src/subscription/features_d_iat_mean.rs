...
impl TrackedFeatures {
    ...
    fn extract_features(&self) -> Vec<f32> {
        ...
        let d_iat_mean = if self.d_pkt_cnt > 0 {
            self.d_last_tsc.saturating_sub(self.syn_ack_tsc) as f32 / *TSC_HZ / self.d_pkt_cnt as f32
        } else {
            -1.0
        };
...
impl Trackable for TrackedFeatures {
    ...
    fn pre_match(&mut self, pdu: L4Pdu, _session_id: Option<usize>, subscription: &Subscription<Self::Subscribed>) {
        ...
        tsc_record!(subscription.timers, "update", t);
    }
    ...
    fn on_terminate(&mut self, subscription: &Subscription<Self::Subscribed>) {
        ...
        tsc_record!(subscription.timers, "extract_features", t);
        let conn = Features {
            // sni: self.sni.clone(),
            features,
        };
