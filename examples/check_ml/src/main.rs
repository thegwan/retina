/// Build: cargo b --features d_ttl_mean,d_ttl_max,d_ttl_med,d_ttl_min,d_winsize_med,d_winsize_min,d_winsize_mean,d_winsize_sum,d_winsize_std,d_ttl_sum,label --bin check_ml
/// Run: sudo env LD_LIBRARY_PATH=$LD_LIBRARY_PATH RUST_LOG=info ./target/debug/check_ml -c configs/offline.toml -m /mnt/netml/datasets/app_class/test/rust_dt.bin -o pred.json
...
fn main() -> Result<()> {
    ...
    let callback = |features: Features| {
        ...
        let res = serde_json::to_string(&(features.sni, pred[0])).unwrap();
